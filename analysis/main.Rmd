...
p9 <- ggplot(donors_ranking_amount_timeline, aes(x = year, y = total_amount, color = recipient)) +
  ...
  scale_color_manual(
    ...
    name = NULL) +
  labs(
    title = "Donors´ amount contribution timeline per recipient") +
  theme_dviz_open() +
  theme(
    axis.line.y.right = element_blank(),
    axis.ticks.y.right = element_blank(),
...
p10 <- ggplot(donors_ranking_amount_timeline_summary, aes(x = year, y = mean, group = year)) +
  ...
  theme_dviz_open() +
    labs(
  title = "Mean donors´ amount contributions timeline") +
...
test <- ggplot(activities_distrib, aes(x = code, y = pct, fill = recipient, labels = pct)) +
  geom_bar(position = "dodge2", stat = "identity", size = 1, width = 1) +
  ...
  labs(title = "Mean donors´ amount contributions timeline")
...
# Stacked bar plot
p11 <- ggplot(activities_distrib, aes(x = code, y = pct, fill = recipient)) +
  geom_col(position = "dodge2", color = "white") +
  geom_text(
    aes(label = pct),
  ...
  scale_x_discrete(expand = c(0, 0), name = NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = TRUE, scale = 1), expand = c(0, 0), breaks = NULL, name = NULL) +
  scale_fill_manual(
    values = c("#f2ac18", "#22577a"),
    breaks = c("Viet Nam", "Bangladesh"),
    labels = c("Viet Nam", "Bangladesh"),

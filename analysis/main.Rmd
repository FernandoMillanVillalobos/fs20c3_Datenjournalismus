...
# Bar plot
p1 <-
  ggplot(donors_contrib, aes(x = fct_reorder(donor, pct), y = pct)) +
  ...
  scale_x_discrete(name = NULL,
                   expand = c(0, 0.5)) +
  labs(title = "Countries/organizations frequency contributions",
       subtitle = "Total WASH contributions for Viet Nam and Bangladesh") +
  coord_flip(clip = "off") +
...
# Bar plot
p2 <-
  ggplot(donors_contrib_per_country, aes(x = fct_reorder(recipient, desc(pct)), y = pct)) +
  geom_col(fill = "#22577a", alpha = 0.9) +
  scale_y_continuous(
    expand = c(0, 0),
    labels = c("0%", "20%", "40%", "60%"),
  ...
  labs(title = "Total donor´s contributions per recipient",
       subtitle = paste("Viet Nam:", donors_contrib_per_country$pct[1],"; Bangladesh:", donors_contrib_per_country$pct[2])) +

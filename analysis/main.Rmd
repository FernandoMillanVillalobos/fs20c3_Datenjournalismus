...
#   select(donor, count, pct))
# 
# (donors_contrib_highlight <- donors_contrib %>%
#     mutate(tohighlight = ifelse(pct %in% c(0.7, 0.5), "yes", "no")))
# 
# # Bar plot
# p1 <-ggplot(subset(donors_contrib_highlight, donors_contrib_highlight$pct > .4), aes(x = fct_reorder(donor, pct), y = pct)) +
#   # geom_col(aes(group = tohighlight), fill = "#22577a", alpha = 0.9) +
...
#                    expand = c(0, 0.5)) +
#   geom_text(data = donors_contrib_highlight[donors_contrib_highlight$pct == c(1)], aes(label = pct), hjust = 1.2, vjust = .3, colour = "white", size = 3) +
...
# Bar plot
p6 <- ggplot(subset(donors_ranking_amount, donors_ranking_amount$pct > 0.1), aes(x = fct_reorder(donor, pct), y = pct)) +
  geom_col(fill = "#22577a", alpha = 0.9) +
  scale_y_continuous(
    expand = c(0, 0.7),
    breaks = seq(0, 30, 2),
...
# Bar plot
p7 <- ggplot(subset(donors_ranking_amount_per_recipient, donors_ranking_amount_per_recipient$pct != 100), aes(x = donor, y = pct, fill = recipient)) +
  ...
  theme(
    ...
    plot.margin = margin(3, 7, 3, 1.5)) #bottom, left, top and right
p7

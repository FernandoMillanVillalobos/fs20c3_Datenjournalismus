...
# Distribution donors total contributions
# (donors_contrib_viz <- df %>% 
#   group_by(donor) %>% 
#   summarize(
#     count = n()
#   ) %>%
...
#         pct = round(freq * 100, 1),
#         pct_white = case_when(
#         pct > 1 ~ pct,     # keep the labels for points you want to highlight
...
# Bar plot
# p1 <-ggplot(donors_contrib, aes(x = fct_reorder(donor, pct), y = pct)) +
...
p3 <- ggplot(donors_contrib_per_donor_recipient, aes(x = count, fill = recipient)) + 
  geom_density_line(alpha = 0.7, kernel = "gaussian") +
  ...
  labs(title = "Total donors´ contributions density per recipient") +
  theme_dviz_hgrid() +
  theme(
...
# Timeline
p5 <- ggplot(donors_contrib_timeline_recipient, aes(x = year, y = count, color = recipient)) + 
  geom_line(size = 1) +
  scale_y_continuous(
    expand = c(0, 0),
    name = "Count") +
...
# Bar plot
p1 <- ggplot(subset(donors_ranking_amount, donors_ranking_amount$pct > 0.1), aes(x = fct_reorder(donor, pct), y = pct)) +
  geom_col(fill = "#22577a", alpha = 0.9) +
  scale_y_continuous(
    expand = c(0, 0.7),
    breaks = seq(0, 30, 2),
...
donors_ranking_amount_per_recipient <- df %>%
  ...
  arrange(pct) %>%
  select(donor, pct, recipient)
# 
# # Distribution of amount per recipient total
# (donors_ranking_amount_per_recipient_total <- donors_ranking_amount_per_recipient %>%
#   filter(pct == 100.00)) %>% 
#   summarize(
#     count = n()
#   ) %>% 
...
# Bar plot
# p2 <- ggplot(bundestag, aes(x = factor(party, levels = bundestag$party), y = seats, fill = party)) + 
#   geom_col() + 
...
# Bar plot
p3 <- ggplot(subset(donors_ranking_amount_per_recipient, donors_ranking_amount_per_recipient$pct != 100), aes(x = fct_reorder(donor, pct), y = pct, fill = recipient)) +
